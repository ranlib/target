...
workflow wf_varpipe_preset {
  input {
    ...
    File config = CONFIG
    File fastq_screen_configuration = FASTQ_SCREEN_CONFIGURATION
    File fastq_screen_contaminants = FASTQ_SCREEN_CONTAMINANTS
    File clockwork_decontamination_metadata = CLOCKWORK_DECONTAMINATION_METADATA
    File clockwork_contaminants = CLOCKWORK_CONTAMINANTS
    File bed = BED
    File json = JSON
  }
  ...
  output {
    ...
    File? polyA_stats   = wf_varpipe.polyA_stats
    File? Ecoli_stats   = wf_varpipe.Ecoli_stats
    File? Covid19_stats = wf_varpipe.Covid19_stats
