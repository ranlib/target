...
task task_bbduk {
  ...
  command <<<
    ...
    out1=~{samplename}.paired_1.fastq.gz out2=~{samplename}.paired_2.fastq.gz
    
    bbduk.sh ~{java_mem} threads=~{threads} \
    in1=~{samplename}.paired_1.fastq.gz in2=~{samplename}.paired_2.fastq.gz \
    out1=~{samplename}.rmadpt_1.fastq.gz out2=~{samplename}.rmadpt_2.fastq.gz \
    ...
    in1=~{samplename}.rmadpt_1.fastq.gz in2=~{samplename}.rmadpt_2.fastq.gz \
    out1=~{samplename}_1.clean.fastq.gz out2=~{samplename}_2.clean.fastq.gz \
    outm=~{samplename}.matched_Covid19.fq \
    ref=contamination/Covid19_ref.fa.gz \
    k=31 hdist=1 \
    stats=~{samplename}.Covid19.stats.txt
    fi
  ...
  output {
    ...
    File? Covid19_stats = "${samplename}.Covid19.stats.txt"
